// 배민 v1.6f: REST API 직접 호출 + networkidle 폐기
const fs = require('fs')
...
const FILES = [
  ['worker/src/adapters/baemin.ts', 'fix(baemin): v1.6f — REST API 직접 호출 (page.context.request.fetch) + networkidle → domcontentloaded'],
  ['worker/Dockerfile',             'chore(worker): CACHE_BUST 20260504T1500 (v1.6f rebuild)'],
]
...
async function main() {
  ...
  console.log('done — v1.6f deployed')
}
