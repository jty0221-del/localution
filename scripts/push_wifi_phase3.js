// Phase 3: Wi-Fi QR + 업체 설정 탭 아이콘 세련화
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'feat: Phase 3 — Wi-Fi QR + refined settings tab icons', content: b64 }
  if (sha) body.sha = sha
...
async function main() {
  await pushFile('app/components/WifiQRBox.tsx', 'app/components/WifiQRBox.tsx')
