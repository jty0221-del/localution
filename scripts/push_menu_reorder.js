// Menu reorder UI: up/down buttons + bulk reorder API
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'feat(menu): up/down reorder buttons + bulk reorder API', content: b64 }
  if (sha) body.sha = sha
...
async function main() {
  await pushFile('app/api/menu/reorder/route.ts', 'app/api/menu/reorder/route.ts')
