// Premium menu redesign: single sophisticated design + store_id matching fix
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'feat(menu): single premium printed-menu design + store_id sync fix', content: b64 }
  if (sha) body.sha = sha
