// Phase 2: 디지털 메뉴판 시스템 (다국어 + 네이버 import)
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'feat: Phase 2 — digital menu board (multi-lang + naver import)', content: b64 }
  if (sha) body.sha = sha
...
async function main() {
  await pushFile('migrations/menu_board.sql', 'migrations/menu_board.sql')
