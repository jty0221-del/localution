// Blog post v3: detail keywords + auto-suggest API + AI draft button
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'feat(blog-post): detail keywords + AI auto-draft for sections 7,8', content: b64 }
  if (sha) body.sha = sha
...
async function main() {
  ...
  await pushFile('app/api/marketing/blog-post-suggest/route.ts', 'app/api/marketing/blog-post-suggest/route.ts')
  console.log('done')
}
