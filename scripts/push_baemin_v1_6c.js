// 배민 v1.6c: 쿠키 JSON 영속화 + post-login URL 판정 개선 + dashboard 재시도
const fs = require('fs')
...
async function main() {
  await pushFile(
    ...
    'worker/src/adapters/baemin.ts',
    'fix(baemin): v1.6c — 쿠키 JSON 영속화 (domain/path/flags 보존) + biz-member/mypage 인증 인식 + dashboard 재시도'
  )
  console.log('done — Worker v1.6c deployed')
}
