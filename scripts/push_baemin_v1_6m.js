// 배민 v1.6m: 30일 경과 리뷰 답글 차단 (배민 자체 정책)
const fs = require('fs')
...
const FILES = [
  ['app/review-admin/components/PlatformReviewAdmin.tsx', 'feat(baemin): v1.6m — 30일 경과 리뷰 답글 발행 UI 차단 + 배지 표시'],
  ['app/api/review-reply/auto-publish/route.ts',          'fix(baemin): v1.6m — 30일 지난 리뷰 서버 차단 (BAEMIN_REPLY_EXPIRED)'],
]
...
async function main() {
  ...
  console.log('done — v1.6m deployed (배민 30일 정책 적용)')
}
