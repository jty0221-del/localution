// PC + mobile readability fixes: max-w containers, decoded URL, responsive padding
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'fix: PC+mobile readability — max-w containers, decoded URL, responsive padding', content: b64 }
  if (sha) body.sha = sha
...
async function main() {
  await pushFile('app/components/MenuQRCard.tsx', 'app/components/MenuQRCard.tsx')
