// 67차 fix: DAYS_BACK 30 → 180 (6개월)
//   · 답글 단 오래된 review 도 수집 (사장님이 30일 이전 답글 단 경우)
//   · 약 4분 fetch 시간 (size=5 + 800ms delay)
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = {
    message: 'fix(coupang): 67차 — DAYS_BACK 30→180 (6개월 fetch, 답글 단 오래된 review 포함)',
    content: b64
  }
  if (sha) body.sha = sha
