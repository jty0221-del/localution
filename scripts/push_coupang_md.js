...
async function pushFile(repoPath, localPath) {
  ...
  const body = {
    message: 'docs(coupang): COUPANGEATS_SYSTEM_v76.md — 5시간 25차 fix 종합 가이드 (DO NOT BREAK)',
    content: b64
  }
  if (sha) body.sha = sha
...
async function main() {
  await pushFile('worker/COUPANGEATS_SYSTEM_v76.md', 'worker/COUPANGEATS_SYSTEM_v76.md')
  console.log('done')
}
