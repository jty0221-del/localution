// Sync menu order between qr-admin and public menu page
// (deterministic ordering: display_order > created_at > id)
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'fix(menu): deterministic order — sync qr-admin and public menu page', content: b64 }
  if (sha) body.sha = sha
...
async function main() {
  await pushFile('app/api/menu/route.ts', 'app/api/menu/route.ts')
