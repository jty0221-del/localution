...
async function pushFile(repoPath, localPath) {
  ...
  const body = {
    message: 'feat(admin): /api/admin/queue-cleanup — 사장님 본인 fetch_reviews 큐 정리 (post_reply 보존)',
    content: b64
  }
  if (sha) body.sha = sha
...
async function main() {
  await pushFile('app/api/admin/queue-cleanup/route.ts', 'app/api/admin/queue-cleanup/route.ts')
  console.log('done')
}
