// 59차 fix: 입력값 검증 + 에러 텍스트 추출 (정확한 실패 사유 진단)
//   · humanType 후 input.value 검증 → 불일치 시 fill() 강제 재입력
//   · login-error-text 실제 텍스트 추출 → 봇 차단 vs 비번 오류 구분
//   · status: credentials_invalid / account_locked / captcha / failed 세분화
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = {
    message: 'fix(coupang): 59차 — 입력값 검증 + login-error-text 추출 (정확한 실패 사유)',
    content: b64
  }
  if (sha) body.sha = sha
