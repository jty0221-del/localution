// 배민 v1.6j_fix: SyntaxError 수정 (|| 와 ?? 섞이지 않게)
const fs = require('fs')
...
const FILES = [
  ['worker/src/adapters/baemin.ts', 'fix(baemin): v1.6j_fix — || ?? syntax error 수정 (Worker startup 실패 차단)'],
  ['worker/Dockerfile',             'chore(worker): CACHE_BUST 20260505T1715 (v1.6j_fix)'],
]
...
async function main() {
  ...
  console.log('done — v1.6j_fix deployed')
}
