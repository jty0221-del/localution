// 2-F: QR 분석 리포트 — recharts 인포그래픽 + PDF 저장
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'feat: 2-F QR analytics report (recharts + insights + PDF export)', content: b64 }
  if (sha) body.sha = sha
...
async function main() {
  await pushFile('app/components/QRReportPanel.tsx', 'app/components/QRReportPanel.tsx')
