// 63차 fix: 답글 발행 silent fail 검증 + 다중 endpoint 후보 시도
//   · 200 응답이라도 body 의 success:false / code:FAIL / error 검사
//   · 5개 endpoint 후보 차례로 시도 (404 는 다음 후보로)
//   · 401/403 즉시 중단, 응답 전체 dump 로깅
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = {
    message: 'fix(coupang): 63차 — 답글 silent fail 검증 + 다중 endpoint 후보',
    content: b64
  }
  if (sha) body.sha = sha
