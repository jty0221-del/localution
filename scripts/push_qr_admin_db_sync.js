// qr-admin DB sync integration: fetch + auto-migrate + sync on create/toggle
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'feat(qr-admin): integrate qr_codes DB sync — auto-migrate LS + sync on create/toggle', content: b64 }
  if (sha) body.sha = sha
