// Fix Quick Access menu link — use real store.slug, not encoded name
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'fix(qr-admin): Quick Access menu link uses storeInfo.slug instead of encoded name', content: b64 }
  if (sha) body.sha = sha
