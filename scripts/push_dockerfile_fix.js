// push Dockerfile fix — remove fragile grep marker check that causes race-condition build fails
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = JSON.stringify({
    message: 'fix: Dockerfile race-safe — drop strict grep marker check',
    content: b64,
    sha
  })
