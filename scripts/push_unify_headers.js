// 헤더 통일 — qr-admin + my 페이지 PageHeader 사용 + lucide 아이콘 마무리
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'refactor: unify headers (qr-admin + my -> PageHeader)', content: b64 }
  if (sha) body.sha = sha
...
async function main() {
  ...
  await pushFile('app/my/page.tsx', 'app/my/page.tsx')
  console.log('done')
}
