// Fix undefined personaTitle reference in blog-post page
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'fix(blog-post): remove undefined personaTitle dep (runtime ReferenceError)', content: b64 }
  if (sha) body.sha = sha
