// Remove bulk translate button (auto-translate happens on menu page lang change)
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'chore(menu): remove bulk translate button (auto on menu page lang change)', content: b64 }
  if (sha) body.sha = sha
