// Phase 2 critical: emoji cleanup (page, qr-admin, customers) + qr-admin break-all → truncate
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'fix: emoji cleanup (page/qr-admin/customers) + qr-admin break-all → truncate', content: b64 }
  if (sha) body.sha = sha
...
async function main() {
  ...
  await pushFile('app/customers/page.tsx', 'app/customers/page.tsx')
  console.log('done')
}
