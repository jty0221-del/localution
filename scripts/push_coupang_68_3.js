// 68차-3: hasReply boolean 우회 — reply_content 또는 raw_snapshot.replies 있으면 답글 박스 노출
//   · DB has_reply 가 false 여도 reply_content 가 있으면 답글 표시
//   · raw_snapshot.replies 배열이 있으면 답글 있음으로 추론
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = {
    message: 'fix(ui): 68차-3 — hasReply 우회 (reply_content/raw_snapshot.replies 보고 추론)',
    content: b64
  }
  if (sha) body.sha = sha
