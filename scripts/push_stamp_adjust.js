// Stamp adjust feature: preview simulator + customer list +/- buttons + API
const fs = require('fs')
...
async function pushFile(repoPath, localPath) {
  ...
  const body = { message: 'feat(stamps): preview simulator + customer +/- adjust buttons + adjust API', content: b64 }
  if (sha) body.sha = sha
...
async function main() {
  await pushFile('app/api/stamps/adjust/route.ts', 'app/api/stamps/adjust/route.ts')
