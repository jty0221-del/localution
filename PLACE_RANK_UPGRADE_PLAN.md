...
| Phase | 상태 | PR | 배포일 | 비고 |
|---|---|---|---|---|
| P0 | **완료 · 배포됨** | #4 | 2026-08-04 | Supabase 마이그레이션 실행 완료. 배포 후 `method` 값 확인 필요 |
| P1 | **코드 완료** | #5 | 2026-08-04 | keyword-rank 목업 제거 → 실데이터 카드 그리드 |
| P2 | 대기 | — | — | |
| P3 | 대기 | — | — | |
| P4 | 대기 | — | — | |
| P5 | 대기 | — | — | |
...
`POST /api/place/keyword-rank` 응답의 `method` 값이 무엇인지 확인한다. 이 값이 파이프라인의 건강 상태를 그대로 보여준다.

| method | 의미 | 조치 |
|---|---|---|
| `map_api` | 정상 — 깊은 순위까지 측정 중 | 없음 |
| `mobile_list` | map 이 막힘, HTML 파싱으로 대체 중 | 관찰 |
| `local_openapi` | 앞의 둘 다 막힘 — **상위 5위까지만 측정 가능** | 개선 필요 |
| `none` | 전부 차단 — Vercel IP 가 네이버에 막힌 상태 | **worker(IPRoyal 프록시 경유)로 이관 필요** |

`none` 이 지속되면 순위 수집을 Vercel 이 아니라 worker 로 옮겨야 한다.
worker 는 이미 IPRoyal 한국 주거용 프록시를 쓰고 있으므로 차단을 우회할 수 있다.

---

## 13. 선제 조치 기록 (P1 과 함께 처리)

나중에 하면 늦는 것들을 미리 처리했다.

### 13-1. `place_snapshots.place_score` 채우기 시작 (2026-08-04)

스키마에만 있고 3년째 비어 있던 컬럼. **데이터는 쌓이는 데 시간이 걸리므로
P3(진단 리뉴얼) 때 채우기 시작하면 그 시점엔 그릴 추이가 없다.**
그래서 P1 시점에 미리 기록을 시작했다.

- `calcStoreScore()` 신설 — 순위 항목을 뺀 매장 자체 체력 점수
  (블로그 25 + 방문자 25 + 평점 10 = 60 을 100 만점으로 재정규화)
- 기록 지점 3곳 모두 반영: 일일 크론 · 수동 새로고침 · 신규 등록
- **키워드별 점수와 직접 비교 금지** — 분모가 다르다

### 13-2. `/api/naver-rank` 고아 라우트 정리

기존 구현이 깨져 있었다: 지역검색 오픈API 를 `start=1,6,11...` 로 페이징했으나
해당 API 는 `start` 최대 1 이라 2페이지부터는 에러. 사실상 상위 5위 전용인데
100위까지 스캔하는 것처럼 보이는 코드였다.

`scanPlaceRank()` 에 위임하도록 교체. 순위 측정 로직의 단일 출처를 유지한다.

### 13-3. 사이드바 라벨 정합

`플레이스(실시간)` → `플레이스 모니터링` (페이지 제목과 일치)
