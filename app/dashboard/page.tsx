        ...
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-2xl p-5 mb-6 shadow-sm shadow-blue-200">
          <div className="flex items-start justify-between">
            <div>
              ...
              <h2 className="text-xl font-bold text-white mt-2 mb-1">좋은 아침이에요, 전태영 사장님! ☀️</h2>
              <p className="text-blue-100 text-sm">오늘 답변 안 된 리뷰가 <span className="font-bold text-white">2개</span> 있어요.</p>
            </div>
            <span className="text-3xl">🤖</span>
          </div>
          <button className="mt-4 bg-white text-blue-600 font-semibold text-sm px-4 py-2 rounded-xl hover:bg-blue-50 transition-colors">AI 답글 바로 달기 →</button>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          {stats.map(stat => (
            <div key={stat.label} className="bg-white rounded-2xl p-4 shadow-sm">
              <div className={`w-9 h-9 rounded-xl ${colorMap[stat.color]} flex items-center justify-center text-lg mb-3`}>{stat.icon}</div>
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          <div className="md:col-span-2 bg-white rounded-2xl p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <a href="/review-admin" className="text-sm text-blue-500 font-medium">전체보기 →</a>
            </div>
              ...
              {reviews.map((review, i) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-gray-50">
                  ...
                  {review.answered
                    ...
                    : <button className="text-xs bg-blue-500 text-white px-3 py-1.5 rounded-lg font-medium whitespace-nowrap shadow-sm shadow-blue-200">AI 답글</button>
                  }
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              {[{ label: '리뷰 응답률', value: 85, color: 'bg-blue-500' }, { label: '고객 재방문율', value: 62, color: 'bg-green-400' }, { label: '매출 달성률', value: 78, color: 'bg-orange-400' }].map(item => (
                <div key={item.label}>
                  ...
                  <div className="h-2 bg-gray-100 rounded-full overflow-hidden"><div className={`h-full ${item.color} rounded-full`} style={{ width: `${item.value}%` }} /></div>
                </div>
              ))}
            </div>
            <div className="mt-5 pt-4 border-t border-gray-100">
              <div className="text-xs text-gray-400 mb-2">이번 달 AI 사용량</div>
              <div className="flex items-end gap-2"><span className="text-2xl font-bold text-gray-900">847</span><span className="text-sm text-gray-400 pb-1">/ 1,000건</span></div>
              <div className="h-2 bg-gray-100 rounded-full overflow-hidden mt-2"><div className="h-full bg-blue-500 rounded-full" style={{ width: '84.7%' }} /></div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {quickActions.map(action => (
              <a key={action.label} href={action.href} className="flex flex-col items-center gap-2.5 p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
