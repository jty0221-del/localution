...
const MOCK_CUSTOMERS = [
  { id: 1, name: '김민수', phone: '010-1234-5678', tag: 'VIP', visits: 12, lastVisit: '2026-04-14', spend: '128,000원' },
  { id: 2, name: '이지영', phone: '010-2345-6789', tag: '단골', visits: 7, lastVisit: '2026-04-12', spend: '74,000원' },
  { id: 3, name: '박서준', phone: '010-3456-7890', tag: '신규', visits: 1, lastVisit: '2026-04-16', spend: '23,000원' },
  { id: 4, name: '최유진', phone: '010-4567-8901', tag: 'VIP', visits: 18, lastVisit: '2026-04-15', spend: '210,000원' },
  { id: 5, name: '정현우', phone: '010-5678-9012', tag: '단골', visits: 5, lastVisit: '2026-04-10', spend: '51,000원' },
  { id: 6, name: '강서연', phone: '010-6789-0123', tag: '신규', visits: 2, lastVisit: '2026-04-11', spend: '34,000원' },
  { id: 7, name: '윤도현', phone: '010-7890-1234', tag: '휴면', visits: 9, lastVisit: '2026-01-20', spend: '93,000원' },
  { id: 8, name: '임소희', phone: '010-8901-2345', tag: 'VIP', visits: 22, lastVisit: '2026-04-13', spend: '287,000원' },
]

type TagKey = 'VIP' | '단골' | '신규' | '휴면' | '블랙리스트'

const TAG_BG: Record<string, string> = {
  'VIP': '#FFF9C4', '단골': '#DCFCE7', '신규': '#EFF6FF', '휴면': '#F2F4F6', '블랙리스트': '#FEE2E2'
}
const TAG_COLOR: Record<string, string> = {
  'VIP': '#B45309', '단골': '#15803D', '신규': '#3182F6', '휴면': '#8B95A1', '블랙리스트': '#DC2626'
}
...
export default function CustomersPage() {
  ...
  const stats = [
    ...
    { label: '재방문 유도', value: customers.filter(c => c.tag === '휴면').length + '명', color: '#F97316', icon: '🔔' },
  ]
    ...
    <div className="p-4 md:p-6 max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-[#F2F4F6] p-4 mb-4">
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
          ...
          <button className="px-4 py-2 bg-[#F2F4F6] text-[#4E5968] text-sm font-medium rounded-xl hover:bg-[#E5E8EB]">
            📨 단체 메시지
          </button>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-[#F2F4F6] overflow-hidden">
        <div className="grid grid-cols-5 px-4 py-2.5 bg-[#F8F9FA] border-b border-[#E5E8EB] text-xs font-semibold text-[#8B95A1]">
          <span>이름</span><span>연락처</span><span>태그</span><span>방문</span><span>마지막 방문</span>
        </div>
        {filtered.map(c => (
          <div key={c.id} className="grid grid-cols-5 px-4 py-3 border-b border-[#F2F4F6] hover:bg-[#F8F9FA] items-center">
            <span className="text-sm font-semibold text-[#191F28]">{c.name}</span>
            <span className="text-sm text-[#4E5968]">{c.phone}</span>
            <span>
              <span className="px-2 py-0.5 rounded-full text-xs font-semibold"
                style={{ backgroundColor: TAG_BG[c.tag] || '#F2F4F6', color: TAG_COLOR[c.tag] || '#4E5968' }}>
                {c.tag}
              </span>
            </span>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6">
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={() => setShowAdd(false)} className="flex-1 py-2.5 border border-[#E5E8EB] rounded-xl text-sm text-[#4E5968]">취소</button>
              <button onClick={() => setShowAdd(false)} className="flex-1 py-2.5 bg-[#3182F6] text-white rounded-xl text-sm font-semibold">추가</button>
            </div>
          </div>
        </div>
      )}
