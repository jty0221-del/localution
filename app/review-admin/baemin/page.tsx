...
export default function ReviewPage_Baemin() {
  ...
  return (
        <div className="p-4 md:p-6 max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-xl font-black text-[#191F28]">배달의민족 리뷰</h1>
              <p className="text-sm text-[#8B95A1] mt-0.5">총 31개 · 별점 4.5 · 미답변 5개</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-5">
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-[#F2F4F6] text-center">
              <p className="text-2xl font-black text-[#191F28]">31</p>
              <p className="text-xs text-[#8B95A1] mt-0.5">전체 리뷰</p>
            </div>
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-[#F2F4F6] text-center">
              <p className="text-2xl font-black" style={{ color: '#2AC1BC' }}>4.5</p>
            ...
            {REVIEWS.map((r, i) => (
              <div key={i} className="bg-white rounded-2xl shadow-sm border border-[#F2F4F6] p-5">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold"
                      style={{ background: '#2AC1BC' }}>{r.name[0]}</div>
                    <div>
                ...
                {r.replied && (r as typeof r & { reply: string }).reply && (
                  <div className="mt-3 p-3 rounded-xl border-l-3" style={{ background: '#EFFFFE', borderLeft: '3px solid #2AC1BC' }}>
                    <p className="text-xs font-semibold mb-1" style={{ color: '#2AC1BC' }}>사장님 답글</p>
              ...
              <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl p-6">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setReplyModal(null)} className="flex-1 py-2.5 text-white rounded-xl text-sm font-semibold hover:opacity-90"
                    style={{ background: '#2AC1BC' }}>등록하기</button>
                </div>
              </div>
            </div>
          )}
