...
export default function NaverReviewPage() {
  ...
  const handleAiReply = async (review: typeof naverReviews[0]) => {
  ...
  return (
    <div style={{ padding: '32px', maxWidth: 960 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
        <div style={{ width: 36, height: 36, borderRadius: 10, background: '#03C75A', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 14 }}>N</div>
        <h1 style={{ fontSize: 24, fontWeight: 700, color: '#fff' }}>네이버 리뷰 관리</h1>
      </div>
      <p style={{ color: '#94a3b8', marginBottom: 24 }}>네이버 플레이스 리뷰를 관리하고 AI 답글을 작성하세요</p>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
        <div style={{ background: 'rgba(3,199,90,0.08)', borderRadius: 12, padding: 16, border: '1px solid rgba(3,199,90,0.15)' }}>
          <p style={{ color: '#94a3b8', fontSize: 13 }}>총 리뷰</p>
          <p style={{ color: '#fff', fontSize: 22, fontWeight: 700 }}>{naverReviews.length}건</p>
        </div>
        <div style={{ background: 'rgba(3,199,90,0.08)', borderRadius: 12, padding: 16, border: '1px solid rgba(3,199,90,0.15)' }}>
          <p style={{ color: '#94a3b8', fontSize: 13 }}>미답변</p>
          <p style={{ color: '#f59e0b', fontSize: 22, fontWeight: 700 }}>{naverReviews.filter(r => !r.replied).length}건</p>
        </div>
        <div style={{ background: 'rgba(3,199,90,0.08)', borderRadius: 12, padding: 16, border: '1px solid rgba(3,199,90,0.15)' }}>
          <p style={{ color: '#94a3b8', fontSize: 13 }}>평균 별점</p>
          <p style={{ color: '#fff', fontSize: 22, fontWeight: 700 }}>{(naverReviews.reduce((s, r) => s + r.rating, 0) / naverReviews.length).toFixed(1)}점</p>
        </div>
      </div>
          ...
          <div key={review.id} style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 14, padding: 20, border: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                {review.hasPhoto && <span style={{ color: '#3b82f6', fontSize: 12 }}>📷 사진</span>}
              </div>
            ...
            {replyingId === review.id && (
              <div style={{ background: 'rgba(3,199,90,0.06)', borderRadius: 10, padding: 14, marginBottom: 12 }}>
                {generating ? (
                  <p style={{ color: '#03C75A' }}>AI 답글 생성 중...</p>
                ) : (
                  <div>
                    <p style={{ color: '#e2e8f0', lineHeight: 1.7, marginBottom: 10 }}>{aiReply}</p>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button style={{ padding: '7px 18px', background: '#03C75A', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>등록</button>
            ...
            {!review.replied && replyingId !== review.id && (
              <button onClick={() => handleAiReply(review)} style={{
                padding: '9px 18px', background: 'linear-gradient(135deg, #03C75A, #00a648)',
