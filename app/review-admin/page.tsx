...
import {
  ...
  ChevronRight, Filter, Search, RefreshCw, Eye,
  ThumbsUp, ThumbsDown, Clock, BarChart2, Share2,
  Instagram, Youtube, Loader2, Sparkles, Bell,
  ArrowUpRight, Camera, FileText, Hash
} from 'lucide-react';

// ── 더미 데이터 ──────────────────────────────────────────────
const reviews = [
  {
    id: 1, platform: '네이버', name: '김지수', rating: 5,
    text: '부천 맛집 찾다가 여기 발견했는데 진짜 맛있어요! 자몽에이드도 맛있고 분위기도 너무 좋아요.',
    time: '10분 전', replied: false, photo: true,
    keywords: ['부천 맛집', '가성비'],
  },
  {
    id: 2, platform: '배민', name: '박민준', rating: 4,
    text: '배달도 빠르고 양도 많아요. 신중동 근처에서 시켰는데 따뜻하게 도착했어요.',
    time: '1시간 전', replied: true, photo: false,
    keywords: ['신중동카페'],
  },
  {
    id: 3, platform: '네이버', name: '이서연', rating: 5,
    text: '회식장소로 딱이에요! 단체석도 있고 음식도 맛있어서 다들 좋아했어요.',
    time: '3시간 전', replied: false, photo: true,
    keywords: ['회식장소', '단체모임'],
  },
  {
    id: 4, platform: '구글', name: 'Sarah K.', rating: 5,
    text: 'Amazing cafe! Great coffee and cozy atmosphere. Definitely coming back!',
    time: '5시간 전', replied: false, photo: false,
    keywords: ['부천 맛집'],
  },
  {
    id: 5, platform: '네이버', name: '최동현', rating: 3,
    text: '음식은 맛있었는데 대기가 좀 길었어요. 그래도 맛은 인정!',
    time: '어제', replied: true, photo: false,
    keywords: [],
  },
];
...
const seoKeywords = [
  { keyword: '부천 맛집', volume: '12,400', competition: '높음', color: 'text-emerald-400' },
  { keyword: '신중동 카페', volume: '3,200', competition: '중간', color: 'text-blue-400' },
  { keyword: '부천 카페', volume: '8,900', competition: '높음', color: 'text-emerald-400' },
  { keyword: '소사구 맛집', volume: '1,100', competition: '낮음', color: 'text-violet-400' },
  { keyword: '부천 회식', volume: '2,300', competition: '중간', color: 'text-blue-400' },
];

const shortformPlatforms = [
  { id: 'tiktok', name: '틱톡', icon: '🎵', size: '9:16 · 60초', color: 'bg-black' },
  { id: 'shorts', name: '유튜브 쇼츠', icon: '▶️', size: '9:16 · 60초', color: 'bg-red-600' },
  { id: 'reels', name: '인스타 릴스', icon: '📸', size: '9:16 · 90초', color: 'bg-gradient-to-r from-purple-500 to-pink-500' },
  { id: 'clip', name: '네이버 클립', icon: '🟢', size: '9:16 · 60초', color: 'bg-emerald-600' },
];
...
export default function ReviewAdminPage() {
  ...
  const generateReply = async () => {
    ...
    const toneGuide: Record<string, string> = {
      warm: '따뜻하고 친근한 사장님 말투, 감사함이 넘치는',
      pro: '격식 있고 신뢰감 있는 비즈니스 말투',
      fun: '유쾌하고 재미있는 개성 있는 말투, 이모지 활용',
    ...
    try {
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `당신은 소상공인 카페 사장님입니다. 아래 고객 리뷰에 ${toneGuide[selectedTone]} 로 답글을 작성해주세요.
      ...
      <div className="p-5 space-y-5 max-w-4xl mx-auto">
                ...
                {['전체', '네이버', '배민', '구글'].map(p => (
                  <button key={p} onClick={() => setFilterPlatform(p)}
                    className={`px-3 py-2 rounded-xl text-xs font-medium transition-all ${
                      filterPlatform === p ? 'bg-violet-600 text-white' : 'bg-white/5 text-gray-400 hover:text-white'
                    }`}>
                    {p}
                  </button>
                ))}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                          ...
                          review.platform === '배민' ? 'bg-blue-500/20 text-blue-400' :
            ...
            {selectedReview && (
              <div className="rounded-2xl bg-[#13131f] border border-violet-500/20 p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-white font-bold text-sm">AI 답글 생성</h3>
                  <span className="text-xs text-gray-500">— {selectedReview.name}님 리뷰</span>
                </div>
                  ...
                  {toneOptions.map(t => (
                    <button key={t.id} onClick={() => setSelectedTone(t.id)}
                      className={`flex items-center gap-2.5 p-3 rounded-xl border text-left transition-all ${
                        selectedTone === t.id ? 'border-violet-500 bg-violet-500/10' : 'border-white/5 bg-white/3 hover:bg-white/5'
                      }`}>
                ...
                <button onClick={generateReply} disabled={isGenerating}
                  className="w-full py-3 rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 disabled:opacity-50 text-white font-bold text-sm transition-all flex items-center justify-center gap-2">
              ...
              <div className="divide-y divide-white/5">
                {seoKeywords.map((kw, i) => (
                  <div key={i} className="px-5 py-3.5 flex items-center gap-4 hover:bg-white/3 transition-all">
                    ...
                    <button className="text-xs bg-violet-600/20 hover:bg-violet-600/40 text-violet-400 px-2.5 py-1.5 rounded-lg transition-all">
                      적용
                    </button>
                  </div>
                ))}
          ...
          <>
                ...
                {shortformPlatforms.map(p => (
                  <button key={p.id}
                    ...
                    className={`flex items-center gap-3 p-3.5 rounded-xl border-2 transition-all text-left ${
                      selectedPlatforms.includes(p.id) ? 'border-violet-500 bg-violet-500/10' : 'border-white/5 bg-white/3'
                    }`}>
                    ...
                    <div className={`w-4 h-4 rounded-full border-2 ml-auto flex-shrink-0 transition-all ${
                      selectedPlatforms.includes(p.id) ? 'border-violet-500 bg-violet-500' : 'border-gray-600'
                    }`} />
                  </button>
                ))}
              </div>
            ...
            <button onClick={handlePublish} disabled={isPublishing || selectedPlatforms.length === 0}
              className={`w-full py-4 rounded-2xl font-bold text-sm transition-all flex items-center justify-center gap-2 ${
                publishDone ? 'bg-emerald-600 text-white' :
                'bg-gradient-to-r from-pink-600 to-rose-600 hover:from-pink-500 hover:to-rose-500 disabled:opacity-50 text-white shadow-lg shadow-pink-500/20'
              }`}>
        ...
        {activeTab === 'menu' && (
          <>
            ...
            <div className="rounded-2xl bg-[#13131f] border border-white/5 p-5">
              ...
              <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
                ...
                <p className="text-white text-sm leading-relaxed">
                  [예시] 신선한 자몽을 직접 착즙해 만든 <span className="text-amber-300 font-medium">부천 카페</span> 대표 음료.
            ...
            <button className="w-full py-4 rounded-2xl font-bold text-sm bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-white shadow-lg shadow-amber-500/20 transition-all flex items-center justify-center gap-2">
