...
function StoreTab() {
  ...
  const handleNaverSync = async () => {
    ...
    if (!query) {
      setSyncError('네이버 플레이스 URL 또는 매장명을 입력해주세요')
      return
    }
    ...
    try {
      ...
      if (!res.ok) {
        setSyncError(data.error || '연동에 실패했습니다')
        return
      }
    ...
    <div className="flex gap-6 items-start">
      ...
      <div className="flex-1 min-w-0 space-y-4">
        ...
        <div className="bg-[#EFF6FF] rounded-2xl p-4 border border-[#BFDBFE]">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleNaverSync} disabled={syncing}
              className={`px-4 py-2.5 rounded-xl text-sm font-bold whitespace-nowrap transition-all ${
                synced && !isMock ? 'bg-green-500 text-white'
          ...
          <p className="text-xs text-[#3182F6] mt-2 opacity-70">
            URL 붙여넣기 또는 매장명으로 검색 → 매장명·주소·전화번호·카테고리 자동 입력
          </p>
        </div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            <div className="w-[38%]">
              <input value={form.mainKeyword} onChange={e => setForm(p => ({ ...p, mainKeyword: e.target.value }))}
                placeholder="메인 키워드"
            ...
            <div className="flex-1">
              <input value={form.subKeywords} onChange={e => setForm(p => ({ ...p, subKeywords: e.target.value }))}
                placeholder="서브 키워드 (쉼표로 구분)"
                className="w-full border border-[#E5E8EB] rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#3182F6] transition-colors" />
        ...
        <div>
          ...
          <textarea value={form.desc} onChange={e => setForm(p => ({ ...p, desc: e.target.value }))} rows={3}
            placeholder="AI 리뷰 답변 작성 시 참고하는 문구입니다. 매장 특징을 입력하세요."
      ...
      <div className="w-72 flex-shrink-0 hidden lg:block">
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden sticky top-8">
          ...
          <iframe
            src={`https://map.kakao.com/link/search/${encodeURIComponent(mapAddress)}`}
            width="100%"
            height="260"
          ...
          <div className="px-4 py-3 space-y-2">
            <a href={`https://map.kakao.com/link/search/${encodeURIComponent(form.address || '')}`}
            ...
            {form.address && (
              <a href={`https://map.naver.com/v5/search/${encodeURIComponent(form.address)}`}
                target="_blank" rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 w-full py-2 rounded-xl bg-[#03C75A] text-white text-sm font-bold hover:bg-[#02A84A] transition-colors">
                네이버지도에서 보기
              </a>
            )}
          </div>
        ...
        <div className="bg-[#FFFBEB] rounded-2xl p-4 mt-4 border border-[#FDE68A]">
          ...
          <p className="text-[11px] text-[#78350F] leading-relaxed mb-2">
            실제 연동을 위해 Vercel 환경변수에 추가해주세요:
          </p>
    ...
    <div className="max-w-xl space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border-l-4 border-[#3182F6]">
        ...
        <div className="pt-4 border-t border-[#F2F4F6]">
          ...
          <div className="flex gap-2">
            {CH.map(ch => (
              <button key={ch.key} onClick={() => setPayChannels(p => ({ ...p, [ch.key]: !p[ch.key] }))}
    ...
    <div className="max-w-xl space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm">
        ...
        <div>
          ...
          <input value={keyword} onChange={e => setKeyword(e.target.value)} placeholder="예: 감사합니다, 방문 감사"
    ...
    <div className="max-w-xl space-y-5">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border-2 border-[#E5E8EB]">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div><p className="font-bold text-[#191F28]">토스페이먼츠 연동</p><p className="text-xs text-[#8B95A1]">정기결제 API 키 설정</p></div>
...
function ShowcaseCarousel({ items }: { items: Array<{ metric: string; label: string; feature: string; who: string; story: string; grad: string }> }) {
  ...
  if (typeof window !== 'undefined') {
    ...
    useState(() => {
      const t = setInterval(() => { setVisible(false); setTimeout(() => { setIdx(i => (i + 1) % items.length); setVisible(true) }, 380) }, 3800)
          ...
          {paymentMethod ? (
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div><p className="font-semibold text-[#191F28] text-sm">{paymentMethod.cardName}</p><p className="text-xs text-[#8B95A1]">**** **** **** {paymentMethod.last4}</p></div>
              </div>
              ...
              return (
                <div key={f.id} className="bg-white rounded-2xl p-5 shadow-sm flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => inCart ? removeFromCart(f.id) : addToCart(f.id)}
      ...
      <div className="w-72 flex-shrink-0 hidden lg:block space-y-4">
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          <h3 className="font-bold text-[#191F28] mb-4 flex items-center gap-2">선택 기능
            {cart.length > 0 && <span className="text-[10px] bg-[#3182F6] text-white rounded-full w-5 h-5 flex items-center justify-center">{cart.length}</span>}
          </h3>
          {cart.length === 0 ? <p className="text-sm text-[#8B95A1] text-center py-4">기능을 선택하면 여기 표시됩니다</p> : (
            <>
          ...
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full">
            ...
            <div className="space-y-4 mb-5">
              <div>
                ...
                <input type="text" value={cardForm.number} onChange={e => setCardForm(p => ({ ...p, number: e.target.value }))}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="block text-xs font-semibold text-[#4E5968] mb-1.5">생년월일</label>
                  <input type="password" placeholder="6자리" value={cardForm.birth} onChange={e => setCardForm(p => ({ ...p, birth: e.target.value }))}
                    className="w-full border border-[#E5E8EB] rounded-xl px-4 py-3 text-sm font-mono focus:outline-none focus:border-[#3182F6] transition-colors" /></div>
              </div>
            </div>
            <button onClick={() => { setPaymentMethod({ cardName: '신한카드', last4: cardForm.number.replace(/\s/g, '').slice(-4) || '1234' }); setShowPaymentModal(false) }}
