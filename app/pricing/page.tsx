...
const features: Feature[] = [
  // 사장님 전용
  { id: 'ai-review', name: 'AI 리뷰 자동 답글', desc: '네이버·배민·쿠팡이츠 리뷰를 AI가 분석하고 맞춤 답글 자동 생성. 하루 평균 5분으로 100% 응답률 달성.', price: 990, icon: '⭐', category: '사장님', popular: true },
    ...
    <div className="min-h-screen bg-[#F2F4F6]">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/login" className="text-sm bg-blue-500 text-white font-semibold px-4 py-2 rounded-xl hover:bg-blue-600 transition-colors">무료 시작</Link>
          </div>
        </div>
      </nav>
      ...
      <div className="pt-24 pb-20 px-4 max-w-6xl mx-auto">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 bg-blue-100 text-blue-600 text-xs font-bold px-3 py-1.5 rounded-full mb-4">
            🛒 내가 쓸 기능만 골라 담기
          </div>
          ...
          <p className="text-gray-500 text-lg max-w-xl mx-auto">
            정해진 요금제 없이 내가 원하는 기능을 골라 담으세요.<br/>
            평균 <span className="font-bold text-blue-500">월 3,000원대</span>로 시작할 수 있어요.
          </p>
        </div>
            ...
            <div className="flex gap-2 mb-5 flex-wrap">
              {(['전체', '사장님', '마케터', '공통'] as const).map(f => (
                <button key={f} onClick={() => setFilter(f)}
                  className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${filter === f ? 'bg-blue-500 text-white shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200'}`}>
              ...
              {filtered.map(feature => {
                ...
                return (
                  <div key={feature.id}
                    ...
                    className={`relative bg-white rounded-2xl p-5 shadow-sm cursor-pointer transition-all border-2 ${inCart ? 'border-blue-500 shadow-blue-100' : 'border-transparent hover:border-gray-200'}`}>
                    {feature.popular && (
                      <div className="absolute -top-2.5 left-4 bg-blue-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">인기</div>
                    )}
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-10 h-10 bg-gray-50 rounded-xl flex items-center justify-center text-xl">{feature.icon}</div>
                        <div>
                      ...
                      <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 mt-1 transition-all ${inCart ? 'bg-blue-500 border-blue-500' : 'border-gray-300'}`}>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-lg font-black text-gray-900">{feature.price.toLocaleString()}원<span className="text-xs font-normal text-gray-400">/월</span></span>
                      <span className={`text-xs font-bold px-3 py-1 rounded-lg transition-all ${inCart ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600'}`}>
                        {inCart ? '✓ 담김' : '+ 담기'}
                      </span>
                    </div>
                  </div>
          ...
          <div className="hidden md:block w-72 sticky top-24">
            <div className="bg-white rounded-2xl shadow-sm p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-gray-900">내 플랜 🛒</h3>
              ...
              {cart.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <div className="text-sm text-gray-400">기능을 골라 담아보세요</div>
                </div>
              ) : (
                <>
                    ...
                    {cartItems.map(item => (
                      <div key={item.id} className="flex items-center justify-between py-2 border-b border-gray-50">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={(e) => { e.stopPropagation(); toggle(item.id) }} className="text-gray-300 hover:text-red-400 text-sm">✕</button>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="border-t border-gray-100 pt-4 mb-4">
                    ...
                    <div className="text-xs text-gray-400 text-right mt-0.5">VAT 포함 · 언제든 변경 가능</div>
                  </div>

                  <Link href="/login"
                    className="block w-full py-3 bg-blue-500 text-white font-bold text-sm rounded-xl hover:bg-blue-600 transition-all shadow-sm shadow-blue-200 text-center">
                    14일 무료로 시작하기 →
                  </Link>
                  <p className="text-[11px] text-gray-400 text-center mt-2">신용카드 불필요 · 무료 체험 후 결제</p>
                </>
              )}
            </div>
            ...
            <div className="bg-blue-50 rounded-2xl p-4 mt-3">
              <div className="text-xs font-bold text-blue-700 mb-2">💡 이런 분들께 추천해요</div>
              <div className="space-y-1.5 text-xs text-blue-600">
                <div>🏪 사장님 → 리뷰+알림톡+정산 = <b>2,970원/월</b></div>
                <div>📣 마케터 → 키워드+블로그+경쟁사 = <b>5,470원/월</b></div>
                <div>🤝 모두 담기 → 전체 기능 = <b>{features.reduce((s,f) => s+f.price, 0).toLocaleString()}원/월</b></div>
              </div>
            </div>
          </div>
        </div>
        ...
        {cart.length > 0 && (
          <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 shadow-lg">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/login" className="bg-blue-500 text-white font-bold text-sm px-6 py-3 rounded-xl hover:bg-blue-600 transition-colors shadow-sm shadow-blue-200">
                무료로 시작하기 →
              </Link>
            </div>
          </div>
