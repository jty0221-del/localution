...
const POSTS = [
  ...
  { id: 3, author: '수원 치킨집', region: '경기 · 수원시', title: '공동구매 - 친환경 포장재 20% 할인 모집합니다', content: '10개 매장이 모이면 친환경 포장재 20% 저렴하게 구매할 수 있어요.', likes: 15, comments: 31, time: '5시간 전', tag: '공동구매' },
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-black text-[#191F28]">
            사장님 커뮤니티 <span className="text-[#EC4899]">{title}</span>
          </h1>
          <p className="text-sm text-[#8B95A1] mt-0.5">소상공인 정보 공유 · 공동구매 · 블랙컨슈머 방어</p>
        </div>
        <button onClick={() => setShowWrite(true)}
          className="px-4 py-2 bg-[#EC4899] text-white text-sm font-semibold rounded-xl hover:bg-[#db2777]">
          + 글쓰기
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {['전체', '네이버플레이스', '배달/배민', '공동구매', '블랙컨슈머', '정부지원금'].map(tag => (
          <button key={tag}
            className="px-3 py-1.5 rounded-full text-xs font-medium bg-white border border-[#E5E8EB] text-[#4E5968] hover:bg-[#FDF2F8] hover:text-[#EC4899] hover:border-[#EC4899]">
            {tag}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-3">
        {POSTS.map(post => (
          <div key={post.id} className="bg-white rounded-2xl shadow-sm border border-[#F2F4F6] p-5 hover:shadow-md transition-all cursor-pointer">
            <div className="flex items-start justify-between gap-3 mb-2">
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 rounded-full text-xs font-semibold"
                  style={{ backgroundColor: (TAG_COLOR_MAP[post.tag] || '#8B95A1') + '20', color: TAG_COLOR_MAP[post.tag] || '#8B95A1' }}>
                  {post.tag}
                </span>
            ...
            <p className="text-xs text-[#8B95A1] mb-3">{post.content}</p>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl p-6">
            ...
            <div className="space-y-3">
              <select className="w-full px-3 py-2 rounded-xl border border-[#E5E8EB] text-sm focus:outline-none focus:border-[#EC4899]">
                ...
                <option>네이버플레이스</option><option>배달/배민</option>
                <option>공동구매</option><option>블랙컨슈머</option><option>정부지원금</option>
              </select>
              ...
              <textarea rows={4} placeholder="내용을 입력해주세요..."
                className="w-full px-3 py-2 rounded-xl border border-[#E5E8EB] text-sm focus:outline-none focus:border-[#EC4899] resize-none" />
            </div>
            <div className="flex gap-2 mt-4">
              <button onClick={() => setShowWrite(false)} className="flex-1 py-2.5 border border-[#E5E8EB] rounded-xl text-sm text-[#4E5968]">취소</button>
              <button onClick={() => setShowWrite(false)} className="flex-1 py-2.5 bg-[#EC4899] text-white rounded-xl text-sm font-semibold">등록</button>
            </div>
          </div>
        </div>
      )}
...
export default function CommunityPage() {
  return (
    <Suspense fallback={<div className="p-6 text-center text-sm text-[#8B95A1]">로딩 중...</div>}>
      <CommunityContent />
    </Suspense>
  )
}
