...
const RAW_JS_TEMPLATE = [
  ...
  '    // 시도할 URL 패턴 (서버가 어떤 형식 받는지 다 시도)',
  '    function buildUrls(p){',
  '      var qs = "page=" + p + "&size=30&statusType=ALL&startDate=" + startDate + "&endDate=" + today;',
  '      return [',
  '        "/api/v1/merchant/reviews/search?storeId=" + storeId + "&" + qs,',
  '        "/api/v1/merchant/stores/" + storeId + "/reviews/search?" + qs,',
  '        "/api/v1/merchant/stores/" + storeId + "/reviews?" + qs,',
  '        "/api/v1/merchant/management/reviews?storeId=" + storeId + "&" + qs',
  '      ];',
  '    }',
  '    var allReviews = [];',
  '    var page = 0;',
  ...
  '    while (page < maxPages) {',
  '      var urls = workingUrl ? [workingUrl(page)] : buildUrls(page).map(function(u){ return u; });',
  '      var json = null;',
  '      var lastStatus = 0;',
  '      var lastBody = "";',
  '      for (var ui = 0; ui < urls.length; ui++) {',
  '        try {',
  '          var res = await fetch(urls[ui], { credentials: "include", headers: fetchHeaders() });',
  '          lastStatus = res.status;',
  '          if (res.ok) {',
  '            json = await res.json();',
  '            if (!workingUrl) {',
  '              var template = urls[ui];',
  '              workingUrl = function(p){ return template.replace(/page=\\d+/, "page=" + p); };',
  '              console.log("로컬루션: 작동하는 URL 패턴 = " + template);',
  '              apiSucceeded = true;',
  '            }',
  '            break;',
  '          } else {',
  ...
  '      }',
  '      // 첫 페이지 API 다 실패 → POST 메서드도 시도',
  '      if (!json && page === 0 && !apiSucceeded) {',
  '        console.log("로컬루션: GET 실패 → POST 메서드 시도");',
  '        var postUrls = buildUrls(0);',
  '        for (var pui = 0; pui < postUrls.length; pui++) {',
  '          try {',
  '            var presBody = JSON.stringify({ storeId: parseInt(storeId, 10), page: 0, size: 30, statusType: "ALL", startDate: startDate, endDate: today });',
  '            var pres = await fetch(postUrls[pui].split("?")[0], {',
  ...
  '              console.log("로컬루션: POST 성공 — " + postUrls[pui]);',
  '              apiSucceeded = true;',
  '              break;',
  '            }',
  '          } catch(_) {}',
